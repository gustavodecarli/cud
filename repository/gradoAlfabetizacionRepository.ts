...
export const fecth = async (page: number, search: string) => {
  const response = await fetch(
    `/api/gradoalfabetizacion?page=${page}&search=${search}`,
    {
      headers: header(),
      method: 'GET'
    }
...
export const save = async (data: BaseEntity) => {
  const response = await fetch('/api/gradoalfabetizacion', {
    headers: header(),
    method: 'POST',
...
export const getOne = async (id: string) => {
  const response = await fetch(`/api/gradoalfabetizacion/${id}`, {
    headers: header(),
    method: 'GET'
  });
